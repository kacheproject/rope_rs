...
async fn router_local_hello_test() {
    ...
    let packet = sock0.recv_packet().await;
...
async fn router_remote_hello_test() {
    ...
    let r0 = Router::new(0, r0_sk.clone());
    let r1 = Router::new(1, r1_sk.clone());
    let sock0 = r0.clone().bind(0, 16).unwrap();
    let port0 = sock0.get_local_port();
    let sock1 = r1.clone().bind(0, 16).unwrap();
    let port1 = sock1.get_local_port();
    let peer0 = r1.new_peer(0, Arc::new(r0_sk.public_key())).unwrap();
    let peer1 = r0.new_peer(1, Arc::new(r1_sk.public_key())).unwrap();
    let transport0 = wires::UdpTransport::new(UdpSocket::bind("[::1]:0").await.unwrap());
    let transport1 = wires::UdpTransport::new(UdpSocket::bind("[::1]:0").await.unwrap());
